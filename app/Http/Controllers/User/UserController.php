...
{
    public function callback_botsms_net()
    {
        if (!isset($_POST['so_tien']) || !isset($_POST['id_khach']) || !isset($_POST['ma_baoMat'])) {
            die('Thiếu tham số bắt buộc');
        }

        $amount = intval($_POST['so_tien']); // số tiền khách chuyển
        $ten_bank = $_POST['ten_bank']; // Tên ngần hàng, ví điện tử
        ...
        if ($ma_baoMat != '5545324534rhhjg4546e453453') {
            die('Mã bảo mật không chính xác. Chặn!');
        }

        $user = User::where('email', 'LIKE',  '%'.$id_khach.'%')->first();
        if (!$user) {
            echo 'Khong tim thay user';
        }

        $old_balance = $user->balance;
        $new_balance = $old_balance + $amount*100;

        $data_his = [
            'user_id' => $user->id,
            'subject' => "Auto $ten_bank: Cộng $amount vào tài khoản, số dư tăng từ $old_balance đ lên $new_balance đ",
            'level' => 0,
            'status' => 1,
            'created_at' => time(),
        ...
        DB::table('v2_ticket')->insert($data_his);
    ...
    public function changePassword(UserChangePassword $request)
    {
        ...
        if (!Helper::multiPasswordVerify(
            ...
            abort(500, __('Mật khẩu cũ là sai'));
        }
    ...
    public function getSubscribe(Request $request)
    {
        ...
        if ($user->plan_id) {
            ...
            if (!$user['plan']) {
                abort(500, __('Gói đăng ký không tồn tại'));
            }
        }
